<template>
  ...
  <div class="container" v-else>
    ...
    <h1 class="main-title"   v-if="region && manager">{{$t('file.cart')}} «{{manager.name}}»</h1>
      ...
      <ul class="cart-items"> 
          <li class="cart-item" :key="product.id" v-for="product in filteredProducts">
            ...
            <div class="cart-item-column cart-item-counter">
              ...
              <div class="cart-item-price-new">{{product.new_price * product.quantity | toCurrency}} {{$t('file.currency')}}</div>
              
              <div class="cart-item-price-old" v-if="product.new_price < product.old_price">{{product.old_price * product.quantity | toCurrency}} {{$t('file.currency')}}</div>
            </div>
          </li> 
        </ul>
    </main>
    <main  class="main" v-if="filteredProducts.length <! 0 && !success">
      <div class="alert">
        ...
        <div class="title alert-title">{{$t('file.cartempty')}}</div>
      </div>
    </main>
      ...
      <div class="card">
        <form class="card-body">
          <div class="form-group">
            <div class="card-title">{{$t('file.checkout')}}</div>
          </div>
          <div class="form-group">
             
            <input  class="form-control" :class="nameError ?'is-invalid' : ''"  @input="nameError = ''"  v-model="form.user.first_name" type="text" :placeholder="$t('file.name')">
              <div v-if="nameError">
                <p style="color:red">{{$t('validation.nameError')}}</p>
              </div>
          </div>
          <div class="form-group">
            <input class="form-control" :class="lastNameError ?'is-invalid' : ''"  @input="lastNameError = ''" v-model="form.user.last_name" type="text" :placeholder="$t('file.last')">
          ...
          <div class="form-group">
            ...
            <input v-on:keypress="isNumber($event)" :class="phoneError ?'is-invalid' : ''" :disabled="user.data && user.data.phone" :maxlength="9" @input="phoneError = ''"  v-model="form.user.phone" class="form-control" type="text" :placeholder="$t('file.phone')">
            </div>
             <div v-if="phoneError">
          ...
          <div class="form-group">
            <input v-model="form.delivery_address_street" class="form-control" type="text" :placeholder="$t('file.street')">
          </div>
          <div class="form-group form-row">
            <div class="col"><input v-model="form.delivery_address_home" class="form-control" type="text" :placeholder="$t('file.home')"></div>
            <div class="col"><input v-model="form.delivery_address_floor" class="form-control" type="text" :placeholder="$t('file.entr')"></div>
            <div class="col"><input v-model="form.delivery_address_apartment" class="form-control" type="text" :placeholder="$t('file.apart')"></div>
          </div>
          <div class="form-group">
            <div class="custom-control custom-radio custom-control-inline">
              ...
              <label class="custom-control-label" for="cart_payment_1">{{$t('file.payment')}}</label>
            </div>
            <!-- <div class="custom-control custom-radio custom-control-inline">
          ...
          <button class="btn btn-block btn-green" @click.prevent="order" type="submit">{{$t('file.req')}}</button>
        </form>
      </div>
    </aside>
    </div>
