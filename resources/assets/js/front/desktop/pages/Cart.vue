<template>
  <div class="content">
    ...
    <div class="container" v-else>
      <div class="main-actions">
        <router-link
          ...
          :to="{name:'home'}"
          exact
        >&#8592; {{$t('pages.back')}}</router-link>
      </div>
      <h1 class="main-title" v-if="region && manager">{{$t('file.cart')}} «{{manager.name}}»</h1>
      <div
        v-if="filteredProducts.length"
      >{{filteredProducts.length}} продуктов на сумму {{cartInfo.total}}</div>
            ...
            <li class="cart-item" :key="product.id" v-for="product in filteredProducts">
              ...
              <div class="cart-item-column cart-item-counter">
                <div class="counter-widget input-group input-group-sm">
                  <div class="input-group-prepend" v-if="product.quantity ==1">
                    <button
                  ...
                  <div class="input-group-append">
                    <button class="btn btn-outline-green" type="button" @click="addToCart(product)">
                      <i class="icon">add</i>
                    </button>
                  </div>
                </div>
                ...
                >{{ product.new_price | toCurrency }} за 1{{product.measure}}</span>
              </div>
              <div class="cart-item-column cart-item-price">
                <div
                  class="cart-item-price-new"
                >{{product.new_price * product.quantity | toCurrency}} {{$t('file.currency')}}</div>

                <div
                  class="cart-item-price-old"
                  v-if="product.new_price < product.old_price"
                >{{product.old_price * product.quantity | toCurrency}} {{$t('file.currency')}}</div>
              </div>
            </li>
          </ul>
        </main>
          ...
          <div class="card">
            <form class="card-body">
              ...
              <div class="form-group">
                <input
                  class="form-control"
                  :class="lastNameError ?'is-invalid' : ''"
                  @input="lastNameError = ''"
                  v-model="form.user.last_name"
                  type="text"
                  :placeholder="$t('file.last')"
                >
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <input
                    ...
                    :disabled="user.data && user.data.phone"
                    :maxlength="9"
                    @input="phoneError = ''"
                    v-model="form.user.phone"
                    class="form-control"
                    type="text"
                    :placeholder="$t('file.phone')"
                  >
                </div>
                <div v-if="phoneError">
              ...
              <div class="form-group">
                <input
                  v-model="form.delivery_address_street"
                  class="form-control"
                  type="text"
                  :placeholder="$t('file.street')"
                >
              </div>
              <div class="form-group form-row">
                <div class="col">
                  <input
                    v-model="form.delivery_address_home"
                    class="form-control"
                    type="text"
                    :placeholder="$t('file.home')"
                  >
                </div>
                <div class="col">
                  <input
                    v-model="form.delivery_address_floor"
                    class="form-control"
                    type="text"
                    :placeholder="$t('file.entr')"
                  >
                </div>
                <div class="col">
                  <input
                    v-model="form.delivery_address_apartment"
                    class="form-control"
                    type="text"
                    :placeholder="$t('file.apart')"
                  >
                </div>
              </div>
              <div class="form-group">
    ...
    <ModalStep
      @fine="proceedOrder"
      :lastName="form.user.last_name"
      :firstName="form.user.first_name"
    ...
    async getProducts() {
      ...
      if (this.cartInfo.prods.length) {
        ...
        let response = await axios.get(
          `/api/managers/${this.manager.slug}/products/cart`,
          {
            params: params
          }
        );
